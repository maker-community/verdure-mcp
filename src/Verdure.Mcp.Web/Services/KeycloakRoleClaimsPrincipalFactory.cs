...
public class KeycloakRoleClaimsPrincipalFactory : AccountClaimsPrincipalFactory<RemoteUserAccount>
{
        ...
        RemoteAuthenticationUserOptions options)
    {
        ...
        if (user.Identity is ClaimsIdentity identity && identity.IsAuthenticated)
        {
            ...
            var clientId = _configuration["Keycloak:ClientId"] ?? "verdure-mcp";
        ...
        try
        {
            ...
            foreach (var claim in jwtToken.Claims.Take(20)) // 限制前20个避免日志过长
            {
                var value = claim.Value.Length > 200
                    ? claim.Value[..200] + "..."
                    : claim.Value;
                _logger.LogDebug("  - {Type}: {Value}", claim.Type, value);
            }
        ...
        string clientId)
    {
        try
        {
            ...
            else
            {
                ...
                _logger.LogWarning(
                    "💡 Tip: Check if Keycloak:ClientId in appsettings.json matches one of the available clients");
            }
