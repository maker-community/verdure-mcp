...
using Verdure.Mcp.Server.Authentication;
...
if (keycloakSettings != null && !string.IsNullOrEmpty(keycloakSettings.Authority))
{
    builder.Services.AddAuthentication(options =>
        {
            // Use policy selector as the default authenticate scheme so each request
            // will execute the selector to choose JwtBearer or ApiToken.
            options.DefaultAuthenticateScheme = "McpSelector";
            options.DefaultScheme = "McpSelector";
            // Keep JwtBearer as the default challenge so WWW-Authenticate remains Bearer when needed
    ...
    // API token scheme (custom handler)
    .AddScheme<AuthenticationSchemeOptions, ApiTokenAuthenticationHandler>("ApiToken", options => { })
    // Policy scheme: choose which underlying scheme to use based on request path
    .AddPolicyScheme("McpSelector", "MCP selector", options =>
    {
        options.ForwardDefaultSelector = context =>
        {
            var path = context.Request.Path.Value ?? string.Empty;
            // If request targets MCP endpoints (contains /mcp) prefer ApiToken
            if (path.EndsWith("/mcp", StringComparison.OrdinalIgnoreCase))
            {
                return "ApiToken";
            }
            // Otherwise use JWT Bearer for admin/API endpoints
...
// - /all/mcp   : All tools
app.MapMcp("/{toolCategory}/mcp").RequireAuthorization();

// Also map root /mcp for backwards compatibility (uses all tools)
app.MapMcp("/mcp").RequireHost("*").RequireAuthorization();
